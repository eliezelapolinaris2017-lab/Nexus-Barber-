...
function resetAppointmentForm(){
  els.appointmentForm.reset(); els.appointmentId.value=''; els.formTitle.textContent='Nueva cita'; els.saveBtn.textContent='Guardar cita'; els.date.value=today;
}
...
els.pinBtn.addEventListener('click', ()=>{
  if(els.pinInput.value.trim() !== settings.pin){ toast('PIN incorrecto'); return; }
...
els.serviceForm.addEventListener('submit', async (e)=>{
  ...
  try{
    await saveUserSettings();
    renderServiceOptions(); renderServices(); resetServiceForm(); toast('Servicio guardado'); setSyncState('ok','Sync activo');
  }catch(err){ toast(`Error Firebase: ${err.message}`); setSyncState('error','Error sync'); }
});
